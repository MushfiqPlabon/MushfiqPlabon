...
GH_USERNAME = os.getenv('GH_USERNAME', 'MushfiqPlabon') # Default to user's GitHub username
OUTPUT_DIR = 'assets'
...
def fetch_user_data(g, username):
    ...
    for repo in repos:
        ...
        try:
            ...
            repo_commits = repo.get_commits(author=user.login, since=datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=365))
...
def generate_summary_text(data):
    summary = []
    summary.append(f"### Recent Activity Summary\n")
    summary.append(f"Hello, I'm {data['user'].name or data['user'].login}!
")
    summary.append(f"In the last year, I've made {len(data['commits'])} commits across my public repositories.\n")
    
    if data['languages']:
        most_used_lang = max(data['languages'], key=data['languages'].get)
        summary.append(f"My most used language is **{most_used_lang}** ({data['languages'][most_used_lang]:.1f}%).\n")
